...
class Circle extends Shape {
    ...
    drawCirclePoints(points, cx, cy, x, y) {
        const pointsToDraw = [
            { x: cx + x, y: cy + y },
            { x: cx - x, y: cy + y },
            { x: cx + x, y: cy - y },
    ...
    drawAntiAliasedPoint(points, cx, cy, x, y, intensity) {
        const pointsToDraw = [
            { x: cx + x, y: cy + y },
            { x: cx - x, y: cy + y },
            { x: cx + x, y: cy - y },
    ...
    fillCircleAntiAliased(points, cx, cy, r) {
...
class Rectangle extends Shape {
    ...
    drawAntiAliased() {
        ...
        if (this.fill) {
            const insidePoints = this.getInsidePoints(this.getVertices());
...
class RegularPolygon extends Shape {
    constructor(x, y, color, sides, config) {
        super({ x, y, color });
        this.sides = sides;
...
class Polygon extends Shape {
    constructor(x, y, color, vertices, fill = true) {
        super({ x, y, color });
    ...
    drawAliased() {
        ...
        if (this.fill) {
            const insidePoints = this.getInsidePoints(this.getTransformedVertices());
            points = points.concat(insidePoints);
        }
        return points;
    }
